...
class ImageAutoencoderConvColor6R7B2N(torch.nn.Module):
    def __init__(self):
        super().__init__()
        #input should be a color image 128x128, 3x128x128
        self.encoder = torch.nn.Sequential(
...
class ImageAutoencoderConvColor6R7C256x256(torch.nn.Module):
    def __init__(self):
        super().__init__()
        #input should be a color image 256x256, 3x256x256
        self.encoder = torch.nn.Sequential(
            torch.nn.Conv2d(3, 32, (3,3), stride=(2,2), padding=(1,1)), # 3x256x256 -> 32x128x128
            torch.nn.ReLU(),
            torch.nn.Conv2d(32, 64, (3,3), stride=(2,2), padding=(1,1)), #32x128x128 -> 64x64x64
            torch.nn.Conv2d(64, 80, (3,3), stride=(1,1), padding=(1,1)), # 64x64x64-> 80x64x64
            torch.nn.ReLU(),
            torch.nn.Conv2d(80, 96,(3,3), stride=(2,2), padding=(1,1)), #80x64x64 -> 96x32x32
            torch.nn.ReLU(),
            torch.nn.Conv2d(96, 128,(3,3), stride=(2,2), padding=(1,1)), # 96x32x32 -> 128x16x16
            torch.nn.ReLU(),
            torch.nn.Conv2d(128, 256,(3,3), stride=(2,2), padding=(1,1)), # 128x16x16 -> 256x8x8
            torch.nn.ReLU(),
            torch.nn.Conv2d(256, 512,(3,3), stride=(2,2), padding=(1,1)), #256x8x8 -> 512x4x4
            torch.nn.ReLU(),
            torch.nn.AvgPool2d(kernel_size=(2,2), stride=(2,2)), # 512x4x4 -> 512x2x2
            PrintShape(),
            torch.nn.ReLU(),
            torch.nn.Flatten(), # Usefull when extracting data from coder
        )
        self.decoder =  torch.nn.Sequential(
            Reshape((-1, 512, 2, 2)), #Because coder flattens the data
            torch.nn.ReLU(),
            torch.nn.ConvTranspose2d(512, 512, (3,3), stride=(2,2), padding=(1,1), output_padding=1), # 512x2x2 -> 512x4x4
            torch.nn.ReLU(),
            torch.nn.ConvTranspose2d(512, 256, (3,3), stride=(2,2), padding=(1,1), output_padding=1), #512x4x4 -> 256x8x8
            torch.nn.ReLU(),
            torch.nn.ConvTranspose2d(256, 128, (3,3), stride=(2,2), padding=(1,1), output_padding=1), #256x8x8 -> 128x16x16
            torch.nn.ReLU(),
            torch.nn.ConvTranspose2d(128, 96,(3,3), stride=(2,2), padding=(1,1), output_padding=1), #128x16x16 -> 96x32x32
            torch.nn.ReLU(),
            torch.nn.ConvTranspose2d(96, 80, (3,3), stride=(2,2), padding=(1,1), output_padding=1), # 96x32x32 -> 80x64x64
            torch.nn.ReLU(),
            torch.nn.ConvTranspose2d(80, 64, (3,3), stride=(1,1), padding=(1,1)), # 80x64x64 -> 64x64x64
            torch.nn.ReLU(),
            torch.nn.ConvTranspose2d(64, 32, (3,3), stride=(2,2), padding=(1,1), output_padding=1), # 64x64x64 -> 32x128x128
            torch.nn.ConvTranspose2d(32, 3, (3,3), stride=(2,2), padding=(1,1), output_padding=1), # 32x128x128 -> 3x256x256
            torch.nn.ReLU(),
            torch.nn.Sigmoid(),
        )
...
class ACC(torch.nn.Module):
    def __init__(self):
        ...
        self.encoder = torch.nn.Sequential(
            torch.nn.Conv2d(3, 32, (3,3), stride=(2,2), padding=(1,1)), # 32x64x64
            torch.nn.ReLU(),
            torch.nn.MaxPool2d(kernel_size=(3,3), stride=(2,2), padding=(1,1)), #32x 32x32
            torch.nn.ReLU(),
            torch.nn.Conv2d(32, 64, (3,3), stride=(2,2), padding=(1,1)), # 96x32x32
            torch.nn.ReLU(),
            torch.nn.Conv2d(64, 128,(3,3), stride=(2,2), padding=(1,1)), # 128x 16x16
            torch.nn.ReLU(),
            torch.nn.Conv2d(128, 256,(3,3), stride=(2,2), padding=(1,1)), # 256x 8x8
            torch.nn.ReLU(),
            torch.nn.Conv2d(256, 512,(3,3), stride=(2,2), padding=(1,1)), # 512x 4x4
            torch.nn.Conv2d(512, 512,(3,3), stride=(1,1), padding=(1,1)), # 512x 2x2
            torch.nn.ReLU(),
            torch.nn.Flatten(), # Usefull when extracting data from coder
        )
        #
        self.decoder =  torch.nn.Sequential(
            Reshape((-1, 512, 2, 2)), #Because coder flattens the data
            torch.nn.ConvTranspose2d(512, 256, (3,3), stride=(2,2), padding=(1,1), output_padding=1), #256x4x4
            torch.nn.ReLU(),
            torch.nn.ConvTranspose2d(256, 128, (3,3), stride=(2,2), padding=(1,1), output_padding=1), #128x8x8
            torch.nn.ReLU(),
            torch.nn.ConvTranspose2d(128, 96, (3,3), stride=(2,2), padding=(1,1), output_padding=1), #128x16x16
            torch.nn.ReLU(),
            torch.nn.ConvTranspose2d(96, 64, (3,3), stride=(2,2), padding=(1,1), output_padding=1), #64x32x32
            torch.nn.ReLU(),
            torch.nn.ConvTranspose2d(64, 32,(3,3), stride=(2,2), padding=(1,1), output_padding=1), #32x64x64
            torch.nn.ReLU(),
            torch.nn.ConvTranspose2d(32, 3, (3,3), stride=(2,2), padding=(1,1), output_padding=1), # 3x128x128
